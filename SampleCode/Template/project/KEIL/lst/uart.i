...
void UART_ClearIntFlag(UART_T *psUART, uint32_t u32InterruptFlag)
{
    ...
    if (u32InterruptFlag & (0x1ul << (10)))       
        psUART->FIFOSTS = (0x1ul << (6)) | (0x1ul << (5)) | (0x1ul << (5)) | (0x1ul << (3));

    if (u32InterruptFlag & (0x1ul << (11)))     
        psUART->MODEMSTS |= (0x1ul << (0));

    if (u32InterruptFlag & (0x1ul << (13)))    
        psUART->FIFOSTS = (0x1ul << (0)) | (0x1ul << (24));
...
void UART_DisableFlowCtrl(UART_T *psUART)
{
    psUART->INTEN &= ~((0x1ul << (12)) | (0x1ul << (13)));
}
...
void UART_Open(UART_T *psUART, uint32_t u32Baudrate)
{
    ...
    uint32_t au32ClkTbl[4] = {(12000000UL), 0, (32768UL), ((48000000UL)/2)};
    uint32_t u32BaudDiv = 0;

     
    u8UartClkSrcSel = (((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL1 & (0x3ul << (24))) >> (24);

     
    u8UartClkDivNum = (((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKDIV0 & (0xful << (8))) >> (8);

     
    psUART->FUNCSEL = (0x0 << (0));

     
    psUART->LINE = (3) | (0x0 << (3)) | (0x0 << (2));

     
    psUART->FIFO &= ~((0xful << (4)) | (0xful << (16)));
    ...
    if (u32Baudrate != 0)
    {
        u32BaudDiv = (((((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1)) + ((u32Baudrate)/2)) / (u32Baudrate))-2);

        if (u32BaudDiv > 0xFFFF)
            psUART->BAUD = ((0) | (((((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1)) + ((u32Baudrate)*8)) / (u32Baudrate) >> 4)-2));
        else
            psUART->BAUD = (((0x1ul << (29)) | (0x1ul << (28))) | u32BaudDiv);
    }
}
...
void UART_SetLine_Config(UART_T *psUART, uint32_t u32Baudrate, uint32_t u32DataWidth, uint32_t u32Parity, uint32_t  u32StopBits)
{
    ...
    if (u32Baudrate != 0)
    {
        u32Baud_Div = (((((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1)) + ((u32Baudrate)/2)) / (u32Baudrate))-2);

        if (u32Baud_Div > 0xFFFF)
            psUART->BAUD = ((0) | (((((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1)) + ((u32Baudrate)*8)) / (u32Baudrate) >> 4)-2));
        else
            psUART->BAUD = (((0x1ul << (29)) | (0x1ul << (28))) | u32Baud_Div);
    }
...
void UART_SetTimeoutCnt(UART_T *psUART, uint32_t u32TOC)
{
    ...
    psUART->TOUT = (psUART->TOUT & ~(0xfful << (0))) | (u32TOC);

     
    psUART->INTEN |= (0x1ul << (11));
}
...
void UART_SelectIrDAMode(UART_T *psUART, uint32_t u32Buadrate, uint32_t u32Direction)
{
    ...
    if (u32Buadrate != 0)
    {
        u32Baud_Div = (((((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1)) + ((u32Buadrate)*8)) / (u32Buadrate) >> 4)-2);

        if (u32Baud_Div < 0xFFFF)
            psUART->BAUD = ((0) | u32Baud_Div);
    }
...
void UART_SelectRS485Mode(UART_T *psUART, uint32_t u32Mode, uint32_t u32Addr)
{
    ...
    psUART->FUNCSEL = (0x3 << (0));

     
    psUART->ALTCTL &= ~((0x1ul << (8)) | (0x1ul << (10)) | (0x1ul << (9)) | (0xfful << (24)));
    psUART->ALTCTL |= (u32Mode | (u32Addr << (24)));
}
...
void UART_SelectLINMode(UART_T *psUART, uint32_t u32Mode, uint32_t u32BreakLength)
{
    ...
    psUART->FUNCSEL = (0x1 << (0));

     
    psUART->ALTCTL &= ~((0x1ul << (7)) | (0x1ul << (6)) | (0xful << (0)));
    psUART->ALTCTL |= (u32Mode | (u32BreakLength << (0)));
}
